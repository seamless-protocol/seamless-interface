...
import {
  DisplayPercentage,
  DisplayPercentageProps,
  DisplayText,
  FlexRow,
  Tooltip,
  Typography,
} from "@shared";
import { useFetchViewSupplyApy } from "../../../statev3/lending-borrowing/hooks/useFetchViewSupplyApy";
import { useFetchViewMaxStrategyApy } from "../../../statev3/loop-strategy/hooks/useFetchViewMaxStrategyApy";
...
export const MaxStrategyApy: React.FC<StrategyApyProps> = ({
  strategy,
  showWarning = true,
  ...rest
}) => {
  const { data: apy, ...restStrategyApy } =
    useFetchViewMaxStrategyApy(strategy);

  if (
    showWarning &&
    apy.value === 0 &&
    !restStrategyApy.isLoading &&
    restStrategyApy.isFetched
  ) {
    return (
      <Tooltip
        tooltip={<Typography type="description">Not enough data</Typography>}
        size="small"
        theme="dark"
      >
...
export const SubStrategyApy: React.FC<SubStrategyApyProps> = ({
  subStrategy,
  showWarning = true,
  ...rest
}) => {
...
export const AssetApy: React.FC<AssetApyProps & { subStrategy?: Address }> = ({
  ...
  if (multiplier && isStrategy)
    return <DisplayText {...rest} viewValue={multiplier} />;

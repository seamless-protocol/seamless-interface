...
import { useFetchViewStrategyRemainingCap } from "../../../statev3/loop-strategy/queries/useFetchStrategyRemainingCap";
import { Address } from "viem";
import { getIsStrategy } from "../../../statev3/settings/configUtils";
...
export const RemainingCap: React.FC<{
  ...
  const { data: remainingCap, ...rest } =
    useFetchViewDetailTotalSupplied(asset);

  return (
    <DisplayText
      viewValue={`${
        remainingCap?.capacityRemainingPercentage?.viewValue || ""
      }% cap available`}
      {...rest}
      {...textProps}
      typography="medium1"
...
export const StrategyRemainingCap: React.FC<{
  ...
  const { data: remainingCap, ...rest } =
    useFetchViewStrategyRemainingCap(asset);

  return (
    <DisplayText
      viewValue={`${
        remainingCap?.remainingCapPercentage?.viewValue || ""
      }% cap available`}
      {...rest}
      {...textProps}
      typography="medium1"
